 ...
 */
int lpm_insert(struct lpm_tree* tree, const char* ip_string, uint32_t netmask)
{
	uint32_t ip;
 ...
 */
int lpm_lookup_string(struct lpm_tree* tree, char* ip_string, char* output)
{
	uint32_t tmp;
	int rt;
...
int lpm_lookup(struct lpm_tree* tree, uint32_t address, char* output)
{
	lookup(address, output, tree->head);
	return 1;
}
